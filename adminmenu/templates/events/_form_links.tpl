                ...
                {foreach $eventLinks as $idx => $link}
                    <div class="bbf-link-entry border rounded p-3 mb-3">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <span class="badge bg-light text-dark">{$link->linkType->label()}</span>
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <select name="links[{$idx}][link_type]" class="form-select form-select-sm">
                                    <option value="external"{if $link->linkType->value === 'external'} selected{/if}>Extern</option>
                                    <option value="internal"{if $link->linkType->value === 'internal'} selected{/if}>Intern</option>
                                    <option value="product"{if $link->linkType->value === 'product'} selected{/if}>Produkt</option>
                                    <option value="category"{if $link->linkType->value === 'category'} selected{/if}>Kategorie</option>
                                    <option value="cms"{if $link->linkType->value === 'cms'} selected{/if}>CMS</option>
                                    <option value="event"{if $link->linkType->value === 'event'} selected{/if}>Event</option>
                                </select>
                            </div>
                            <div class="col-md-5">
                                <label class="form-label">URL / Ziel</label>
                                <input type="text" name="links[{$idx}][target_url]" value="{$link->targetUrl|default:''|escape:'html'}" class="form-control form-control-sm">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Label (DE)</label>
                                <input type="text" name="links[{$idx}][label_ger]" value="{$link->getLabel()|escape:'html'}" class="form-control form-control-sm">
                            </div>
                        </div>
