                ...
                {foreach $eventMedia as $idx => $media}
                    <div class="bbf-media-entry border rounded p-3 mb-3">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <span class="badge bg-light text-dark">{$media->mediaType->label()}</span>
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <select name="media[{$idx}][media_type]" class="form-select form-select-sm">
                                    <option value="image"{if $media->mediaType->value === 'image'} selected{/if}>Bild</option>
                                    <option value="gallery"{if $media->mediaType->value === 'gallery'} selected{/if}>Galerie</option>
                                    <option value="youtube"{if $media->mediaType->value === 'youtube'} selected{/if}>YouTube</option>
                                    <option value="vimeo"{if $media->mediaType->value === 'vimeo'} selected{/if}>Vimeo</option>
                                    <option value="local_video"{if $media->mediaType->value === 'local_video'} selected{/if}>Lokales Video</option>
                                    <option value="download"{if $media->mediaType->value === 'download'} selected{/if}>Download</option>
                                </select>
                            </div>
                            <div class="col-md-5">
                                <label class="form-label">Datei / URL</label>
                                <input type="text" name="media[{$idx}][file_path]" value="{$media->filePath|default:$media->externalUrl|escape:'html'}" class="form-control form-control-sm">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Alt-Text / Titel</label>
                                <input type="text" name="media[{$idx}][alt_text]" value="{$media->altText|default:''|escape:'html'}" class="form-control form-control-sm">
                            </div>
                        </div>
